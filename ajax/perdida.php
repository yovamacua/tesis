<?php 
	...
	$idperdidas = isset($_POST["idperdidas"]);
...
switch ($_GET["op"]) { 
	...
	case 'guardaryeditar':
		$datos = $perdidas->get_perdidas_por_id($_POST["idperdidas"]);
	       	/*si el titulo no existe entonces lo registra
	        importante: se debe poner el $_POST sino no funciona*/
	        if(empty($_POST["idperdidas"])){
			    ...
			    if(is_array($datos)==true and count($datos)==0){
	             ...
	             $perdidas-> editar_perdida($idperdidas, $nombreProduc, $cantidad, $descripcion, $precioProduc, $mes, $anio, $unidadDelProduc, $id_usuario);
	 ...
	 //mensaje error
		...
		case 'mostrar':
			...
			//el parametro idperdidas se envia por AJAX cuando se edita la perdida
			$datos = $perdidas->get_perdidas_por_id($_POST["idperdidas"]);
			if(is_array($datos)==true and count($datos)>0){
		...
		case 'listar':
		    ...
		    foreach($datos as $row){
		        $sub_array = array();
		     	...
		     	$sub_array[] = '<button type="button" onClick="mostrar('.$row["idperdidas"].');"  id="'.$row["idperdidas"].'" class="btn btn-warning btn-md update"><i class="glyphicon glyphicon-edit"></i> Editar</button>';
		     	$sub_array[] = '<button type="button" onClick="eliminar('.$row["idperdidas"].');"  id="'.$row["idperdidas"].'" class="btn btn-danger btn-md"><i class="glyphicon glyphicon-edit"></i> Eliminar</button>';
		      	$data[] = $sub_array;
		      }
		...
		case "eliminar_perdida":
		    ...
		    if(is_array($datos)==true and count($datos)>0){
		          $perdidas->eliminar_perdida($_POST["idperdidas"]);
